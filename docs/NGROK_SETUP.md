...
## 🔗 Paso 5: Actualizar Configuración en Flow y Backend
...
# Ejemplo real:
# FLOW_RETURN_URL=https://troublesome-trinity-subvitreously.ngrok-free.dev/api/payments/return
```

### 3. Backend

El backend ya está configurado para manejar las URLs de ngrok automáticamente.
...
2. **Terminal 2**: Ngrok activo
   ```bash
   ngrok http 3000
   ```

3. **Terminal 3**: Frontend corriendo
   ```bash
   cd frontend
   npm run dev
   ```
...
4. Si funciona, el problema está en Flow
...
- **Opción 2**: Usa plan de pago de ngrok para URL fija
- **Opción 3**: Usa variable de entorno y actualiza `.env`
...
### URL permanente (Plan Pro)

Si usas la versión de pago de ngrok, obtienes URL fija:
```
https://mi-app.ngrok.io
```

No necesitas actualizar `.env` cada vez.

---

## 🚀 Script Automático para Windows

Crea archivo `start-ngrok.bat`:

```batch
@echo off
REM Cambiar a carpeta de ngrok
cd C:\ngrok

REM Iniciar ngrok en puerto 3000
ngrok http 3000
```

Luego doble-click para ejecutar.

---

## 📊 Estado Actual de AMANWAL

```
Backend:       ✅ Corriendo en localhost:3000
Frontend:      ✅ Corriendo en localhost:5173
Ngrok:         ✅ Tuneleo configurado
Flow.cl:       ✅ Integrado y funcional
Pagos:         ✅ En proceso
```
...
5. ✅ Actualiza Flow.cl y backend/.env
6. ✅ Prueba un pago
7. 🎉 ¡Listo!
